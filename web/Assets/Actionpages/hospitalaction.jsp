<%-- 
    Document   : hospitalaction
    Created on : 8 Feb, 2022, 11:24:43 PM
    Author     : rinus
--%>
...
//checking if request cotains multipart data
            ...
            //f_item=(FileItem)itr.next();
                        ...
                        if (field_name.equals("txtimg"))//write the name of image here it is img_product
                        {
                            String ext = file_name.substring(file_name.lastIndexOf("."));//means the extension is stored to variable ext
                            //setting path to store image
                            File proj_path = new File(config.getServletContext().getRealPath("/"));
                            String file_path = proj_path.getParentFile().getParentFile().getPath() + "\\Web\\Assets\\Photo\\";//path for storing image
                            Random r = new Random();//to generate random number
                            int r_num = r.nextInt(1111) + 999;//values between this are generated

                            photo = "uset_" + r_num + ext;//generating random number for images so this name and format of imagewil be stored inside the server
                       ...
                       if (field_name.equals("txtproof"))//write the name of image here it is img_product
                        {
                            ...
                            proof = "uset_" + r_num + ext;//generating random number for images so this name and format of imagewil be stored inside the server
                            //creating a file object
...
//                        }
           ...
           //to upload proof of doctor
                ...
                {//Strinh str1="insert into tbl_user ()";//inserting file
                 String InsQry = "insert into tbl_hospital(hospital_name,hospital_contact,hospital_email,hospital_address,hospitaltype_id,place_id,hospital_logo,hospital_proof,hospital_pswrd,hospital_doj)values('"+value[0]+"','"+value[1]+"','"+value[2]+"','"+value[3]+"','"+value[4]+"','"+value[6]+"','"+photo+"','"+proof+"','"+value[7]+"',curdate())";
   ...
   //System.out.println(InsQry);
        ...
        <script type="text/javascript">
            ...
            setTimeout(function() {
                window.location.href = '../../Guest/hospitalreg.jsp';
